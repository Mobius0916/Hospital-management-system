...
void doctor::fillMap(){
    ...
    while (getline(f >> ws, temp)){
        ...
        d.age = strToNum(s5);
        d.add.strToAdd(s7);
        d.last_hh = strToNum(s9);
        d.appointmentsBooked = strToNum(s10);
...
void doctor::saveMap()
{
    ...
    f << "doctorId,firstName,lastName,gender,age,mobNumber,address,type,Lastbooktime,appointmentsBooked\n";
    for (auto i : hospital::doctorsList)
        f << i.second.id << "," << i.second.firstName << "," << i.second.lastName << "," << i.second.gender
          ...
          << "," << i.second.type << "," << i.second.last_hh  << "," << i.second.appointmentsBooked<< endl;
    f.close();
...
void doctor::addPerson()
{
    ...
    f << endl;
    f << firstName << "," << lastName << "," << gender << "," << age << "," << mobNumber << "," << add.addToStr() << "," << type << ",N,NA";
    f.close();
